...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char target[last_sp - first_sp];
            strncpy(target, first_sp + 1, last_sp - first_sp - 1);
            target[last_sp - first_sp - 1] = '\0';
            ...
            // target may not contain '"'
            for (int i = 0, n = strlen(target); i < n; i++)
            {
                if (target[i] == '"')
                {
                    error(400);
            ...
            // validate version (MUST be HTTP 1.1; HTTP 2.0 getting no respect)
            needle = strstr(line, "\r\n");
            char version[needle - last_sp];
            strncpy(version, last_sp + 1, needle - last_sp - 1);
            version[needle - last_sp - 1] = '\0';
            ...
            char* q_mark = strchr(target, '?');
            char* null_term = strchr(target, '\0');
            int q_length = 0;
            
            if (q_mark != NULL)
            {
                // if ? exists in the target, set query length
                q_length = null_term - q_mark - 1;
            }
            
            char query[q_length + 1];
            strncpy(query, q_mark + 1, q_length);
            query[q_length] = '\0';

            // concatenate root and absolute-path
            char path[strlen(root) + strlen(target) + 1];
            strcpy(path, root);
            // concat up to ? in target
            strncat(path, target, q_mark - target);
            ...
            needle = strrchr(path, '.');
            char* p_end = strchr(path, '\0');
            char extension[p_end - needle];
            strncpy(extension, needle + 1, p_end - needle - 1);
            extension[p_end - needle - 1] = '\0';
...
const char* lookup(const char* extension)
{
    ...
    char mime_type[len + 1];
    for (int i = 0; i < len; i++)
    {
        mime_type[i] = tolower(extension[i]);
    }
    mime_type[len] = '\0';
    
    // return the appropriate MIME type
    if (strcmp(mime_type, "css") == 0)
        return "text/css";
    else if (strcmp(mime_type, "html") == 0)
        return "text/html";
    else if (strcmp(mime_type, "gif") == 0)
        return "image/gif";
    else if (strcmp(mime_type, "ico") == 0)
        return "image/x-icon";
